...
vm.runInContext(fs.readFileSync(path.join(root, "history-review-data.js"), "utf8"), context);
...
const historyReviewData = context.window.HISTORY_REVIEW_DATA;
...
for (const wallpaperFeature of ["living-constitution", "civic-word-drift", "living-current-word"]) {
  if (!fs.readFileSync(path.join(root, "styles.css"), "utf8").includes(wallpaperFeature)) errors.push(`The Living Constitution background is missing: ${wallpaperFeature}`);
}
for (const wallpaperScriptFeature of ["renderLivingConstitution", "WE THE PEOPLE", "currentWord.textContent = current.title"]) {
  if (!fs.readFileSync(path.join(root, "app.js"), "utf8").includes(wallpaperScriptFeature)) errors.push(`The Living Constitution background behavior is missing: ${wallpaperScriptFeature}`);
}
...
const expectedUnitTitles = ["First Bell", "Foundations of American Democracy", "The Three Branches at Work", "Election Season", "Checks and Balances", "Rights in Real Life", "Government Around the World", "Constitutional Tensions"];
...
if (!html.includes("styles.css?v=20260920-election-language") || !html.includes("app.js?v=20260920-election-language") || !html.includes("course-data.js?v=20260916-slide-labels") || !html.includes("foundations-data.js?v=20260823-unit-1-launch")) {
...
const historyReviewIds = historyReviewData?.checkpoints?.map(checkpoint => checkpoint.id).join("|");
...
if (electionData.location?.exampleZip !== "92114" || electionData.location?.district !== "CALIFORNIA DISTRICT 52" || !electionData.location?.lookupSource?.includes("sdvote.com")) {
  errors.push("The 2026 ballot must use 92114 only as the District 52 example and link to the San Diego County exact-ballot lookup.");
}
...
if (!html.includes("election-2026-data.js?v=20260920-election-language")) errors.push("The updated election data needs the current cache version.");
for (const electionRendererFeature of ["WHAT CHANGES?", "COST OR SAVINGS", "READ THE OFFICIAL VOTER GUIDE", "START WITH YOUR ZIP CODE", "FIND MY EXACT BALLOT", "election-zip-form"]) {
...
for (const removedElectionInstruction of ["These short backgrounds describe public experience", "Open one card. Read what changes"]) {
  if (appCode.includes(removedElectionInstruction)) errors.push(`Unnecessary election-page instruction remains: ${removedElectionInstruction}`);
}
for (const bondProposition of [1, 38]) {
  if (!electionData.propositions.find(proposition => proposition.number === bondProposition)?.explanation.includes("A bond lets the state borrow money and repay it over time.")) errors.push(`Proposition ${bondProposition} must define a bond where students need the term.`);
}
...
if (!Array.isArray(config.upcoming)) errors.push("Upcoming activities must be a list, which may be empty.");
...
if (!html.includes("cp-rosters.js?v=20260826-exit-ticket") || !html.includes("app.js?v=20260920-election-language") || !html.includes("styles.css?v=20260920-election-language")) errors.push("Exit-ticket cache versions are not current.");
...
for (const folder of ["", "documents", "cases", "prove-your-case"]) {
  for (const file of fs.readdirSync(path.join(root, folder)).filter(name => name.endsWith(".html"))) {
    const page = fs.readFileSync(path.join(root, folder, file), "utf8");
